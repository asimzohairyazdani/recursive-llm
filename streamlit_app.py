...
def run_recursive_demo(
    ...
    client = OllamaClient(ollama_url, mock=mock)
...
with st.sidebar:
    ...
    max_children = st.slider("Max children per node", min_value=1, max_value=5, value=3)
    mock = st.toggle("Mock mode", value=False)

    st.info(
        "Start with depth 1 for local models. Depth multiplies calls quickly."
    )
...
if run_button:
    ...
    with st.status("Running recursive loop...", expanded=True) as status:
        st.write("Planning subtasks, solving nodes, critiquing answers.")
        try:
